...
{
	private static final long serialVersionUID = -7851497909260695118L;
	...
	private void createInteractivePanel()
	{
		...
		dataSources[0] = new DataSeries("Total (events/second)", data, 0, 1);
		dataSources[1] = new DataSeries("Incoming (events/second)", data, 0, 2);
		dataSources[2] = new DataSeries("Outgoing (events/second)", data, 0, 3);
	...
	public void update(Integer incomingEvents, Integer outgoingEvents)
	{		
		Integer totalEvents = incomingEvents + outgoingEvents;
		...
		data.set(1, Graphics.X_RANGE, totalEvents);
		data.set(2, Graphics.X_RANGE, incomingEvents);
		data.set(3, Graphics.X_RANGE, outgoingEvents);
