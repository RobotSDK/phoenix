...
{
	private static final long serialVersionUID = 6738461846115078705L;
	...
	private void createInteractivePanel()
	{
		...
		dataSources[0] = new DataSeries("Total matches (events/second)", data, 0, 1);
		dataSources[1] = new DataSeries("Positive matches (events/second)", data, 0, 2);
		dataSources[2] = new DataSeries("Negative matches (events/second)", data, 0, 3);
	...
	public void update(Integer positiveMatches, Integer negativeMatches)
	{
		Integer totalMatches = positiveMatches + negativeMatches;
		...
		data.set(1, Graphics.X_RANGE, totalMatches);
		data.set(2, Graphics.X_RANGE, positiveMatches);
		data.set(3, Graphics.X_RANGE, negativeMatches);
