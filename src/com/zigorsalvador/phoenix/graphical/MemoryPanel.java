...
{
	private static final long serialVersionUID = 1675678847118364999L;
	...
	private void createInteractivePanel()
	{
		...
		dataSources[0] = new DataSeries("Memory utilization (MBytes)", data, 0, 1);
			
		Color[] dataColors = new Color[1];
		
		dataColors[0] = new Color(0, 200, 0);
		...
		panel = PanelFactory.create(dataSources, dataColors, dataFills, memory);
	...
	public void update(Float memoryUtilization)
	{
		...
		data.set(1, Graphics.X_RANGE, memoryUtilization);
		
		plot.getAxis(XYPlot.AXIS_Y2).setRange(0, memory * 1.1);
		plot.getAxis(XYPlot.AXIS_Y).setRange(0, memory * 1.1);	
